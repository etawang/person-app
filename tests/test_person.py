...
class TestPerson(TestCase):
    ...
    def test_list_all_persons(self):
        ...
        assert set(map(lambda p: p['id'], response.json)) == set(range(1, self.NUM_PERSONS + 1))
    ...
    def test_read_person_version(self):
        for version in range(1, self.NUM_PERSONS):
            ...
            assert response.status_code == 200
            assert response.json['person_id'] == self.NUM_PERSONS
    ...
    def test_put_person(self):
        ...
        p_archive = PersonArchive.query.filter_by(person_id=1, version=p.version).first()
        assert p_archive.first_name == p.first_name
        assert p_archive.version == p.version
        p_archive_prev = PersonArchive.query.filter_by(person_id=1, version=old_version).first()
