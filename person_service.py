...
class PersonService:
    ...
    def read(self, person_id):
        return Person.query.get_or_404(person_id).to_dict()

    def read_all(self):
        return list(map(lambda p: p.to_dict(), Person.query.all()))

    def read_version(self, person_id, version):
        return PersonArchive.query \
            .filter_by(person_id=person_id, version=version) \
            .first_or_404() \
            .to_dict()
